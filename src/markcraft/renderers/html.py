...
from markcraft.tokens.block import HtmlBlock
from markcraft.tokens.span import HtmlSpan
from markcraft.renderers.base import BaseRenderer
from markcraft.renderers.base import URI_SAFE_CHARACTERS
...
class HtmlRenderer(BaseRenderer):
    ...
    See markcraft.renderers.base module for more info.
    """
    def __init__(
        self,
        *extras,
    ...
    def render_raw_text(self, token: span.RawText) -> str:
        return self.escape_html_text(token.content)

    @staticmethod
    def render_html_span(token: span.HtmlSpan) -> str:
    ...
    def render_table_row(self, token: block.TableRow, is_header=False) -> str:
        template = '<tr>\n{inner}</tr>\n'
        inner = ''.join([self.render_table_cell(child, is_header)
                         for child in token.children])
        return template.format(inner=inner)

    def render_table_cell(self, token: block.TableCell, in_header=False) -> str:
    ...
    @staticmethod
    def render_thematic_break(token: block.ThematicBreak) -> str:
        return '<hr />'

    @staticmethod
    def render_line_break(token: span.LineBreak) -> str:
        return '\n' if token.soft else '<br />\n'

    @staticmethod
    def render_html_block(token: block.HtmlBlock) -> str:
        return token.content
