...
from markcraft.tokens.core import MatchObj


class Program(block.BlockToken):
    def __init__(self, lines):
        self.children = span.tokenize_inner(''.join([line.strip() for line in lines]))


class Expr(span.SpanToken):
    @classmethod
    def find(cls, string):
        matches = []
        start = []
...
class Whitespace(span.SpanToken):
    parse_inner = False
...
class Scheme(BaseRenderer):
    def __init__(self):
        ...
        self._span_token_types = [Expr, Number, Variable, Whitespace]
