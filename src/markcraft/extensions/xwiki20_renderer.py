...
from markcraft.renderers.base import BaseRenderer, URI_SAFE_CHARACTERS
...
class XWiki20Renderer(BaseRenderer):
    ...
    def __init__(self, *extras):
        ...
        localExtras = [block.HtmlBlock, span.HtmlSpan, span.XWikiBlockMacroStart, span.XWikiBlockMacroEnd]
    ...
    def render_inner(self, token):
        if isinstance(token, block.List):
            if token.start:
        ...
        wrap = False
        if isinstance(token, block.BlockToken) and len(token.children) > 1:
            ...
            for child in token.children[1:]:
                if isinstance(token, block.ListItem) and not isinstance(child, block.List):
                    # Note: Nested list within a list item is OK, because it does its own wrapping if necessary.
                    wrap = True
                    break
                if isinstance(token, (block.TableCell)) and isinstance(child, block.BlockToken):
